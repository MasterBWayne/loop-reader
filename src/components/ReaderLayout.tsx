...
export function ReaderLayout({
  ...
  return (
    <div className="h-screen flex flex-col bg-navy">
      ...
      {/* Top bar */}
      <header className="bg-navy border-b border-ink/10 px-4 py-3 flex items-center justify-between shrink-0 z-30 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <button onClick={() => setShowNav(!showNav)} className="text-muted-soft hover:text-ink transition-colors p-1">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-muted-soft mr-2 hidden sm:block">{currentChapter + 1} of {chapters.length}</span>
          {/* Feature 4: Living Summary button */}
          {user?.id && (
            <button
              onClick={() => setShowPersonalSummary(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-ink/5 text-muted-soft hover:bg-ink/10 hover:text-ink transition-all hidden sm:flex"
            >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${showChat ? 'bg-gold text-white' : 'bg-ink/5 text-muted-soft hover:bg-ink/10 hover:text-ink'}`}
          >
        ...
        <button
          ...
          className={`px-4 py-2 text-xs font-medium transition-all border-b-2 ${
            activeTab === 'chapters' && !showExerciseHistory
              ...
              : 'text-muted-soft border-transparent hover:text-ink'
          }`}
        >
          Chapters
        </button>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        {showNav && (
          <>
            ...
            <aside className="absolute md:relative left-0 top-0 bottom-0 w-64 bg-navy border-r border-ink/10 z-20 overflow-y-auto">
              <div className="p-4">
                {onBackToLibrary && (
                  <button
                ...
                <p className="text-[11px] text-muted-soft font-semibold tracking-[0.1em] uppercase mb-4">Chapters</p>
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-colors ${
                          locked
                            ...
                            : 'text-muted-soft hover:text-ink hover:bg-ink/5'
                        }`}
                      >
        ...
        {activeTab === 'practice' && user?.id && (
          <div className={`flex-1 overflow-y-auto reader-scroll bg-navy ${showChat ? 'hidden md:block' : ''}`}>
            <HabitTracker bookId={bookId} bookTitle={bookTitle} userId={user.id} />
          </div>
        )}
        {activeTab === 'practice' && !user?.id && (
          <div className="flex-1 flex items-center justify-center bg-navy text-muted-soft text-sm">
        ...
        {showReadTimeToast && chapterReadTime && chapterReadTime > 5 && (
          <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-navy-light border border-gold/30 rounded-xl px-5 py-3 shadow-lg animate-message-in flex items-center gap-2">
            ...
            <button onClick={() => setShowReadTimeToast(false)} className="text-muted-soft hover:text-muted-soft ml-2">
        ...
        <div className={`flex-1 overflow-y-auto reader-scroll transition-all duration-300 ${activeTab !== 'chapters' || showExerciseHistory ? 'hidden' : ''} ${showChat ? 'hidden md:block' : ''}`}>
          {unlocked ? (
            <article className="max-w-[740px] mx-auto px-4 md:px-6 py-12">
              ...
              <div className="mb-8" style={{ marginBottom: '32px' }}>
                <div className="flex items-center gap-3 mb-3">
                  <p className="text-[11px] text-muted-soft font-semibold tracking-[0.1em] uppercase">Chapter {chapter.number}</p>
              ...
              {/* Content */}
              <div className="text-[17px] text-ink max-w-[680px]" style={{ fontFamily: "var(--rk-font-body)", lineHeight: '1.8' }}>
              ...
              {pendingFollowUp && !followUpDismissed && pendingFollowUp.chapter_number === chapter.number && (
                <div className="mt-10 bg-navy-light border border-gold/30 rounded-xl p-5 pl-7 border-l-[3px] border-l-gold animate-message-in">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <span className="text-lg">🎯</span>
                      <span className="text-[11px] font-semibold text-muted-soft uppercase tracking-[0.1em]">Check-in</span>
                    </div>
                    <button onClick={() => setFollowUpDismissed(true)} className="text-muted-soft hover:text-ink text-xs">Later</button>
                  </div>
                  <p className="text-sm text-muted mb-2" style={{ fontFamily: "var(--rk-font-heading)" }}>
                    You committed: <em className="text-gold-light">"{pendingFollowUp.commitment_text}"</em>
                  </p>
                  <p className="text-sm text-ink font-medium mb-3" style={{ fontFamily: "var(--rk-font-heading)" }}>
                    What happened?
                  </p>
                  <textarea
                    value={followUpInput}
                    ...
                    className="w-full bg-navy-light border border-ink/10 rounded-lg px-4 py-3 text-sm text-ink placeholder:text-muted/40 outline-none focus:border-gold transition-colors resize-none leading-relaxed disabled:opacity-50"
              ...
              <div className="mt-16 pt-8 border-t border-ink/10 flex items-center justify-between">
                <button
                  ...
                  className="flex items-center gap-2 text-sm font-medium text-muted-soft hover:text-ink disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                >
                ...
                <span className="text-xs text-muted-soft">{currentChapter + 1} / {chapters.length}</span>
                {currentChapter < chapters.length - 1 ? (
                  isChapterUnlocked(chapters[currentChapter + 1].number, progress) ? (
                    <button
                      onClick={() => navigateChapter(currentChapter + 1)}
                      className="flex items-center gap-2 text-sm font-medium text-muted-soft hover:text-ink transition-colors"
                    >
                      Next
                  ...
                  <span className="text-xs text-muted-soft">Final chapter</span>
                )}
              </div>
            </article>
          ) : (
            ...
            <div className="flex flex-col items-center justify-center h-full px-6 text-center">
              <div className="w-16 h-16 bg-navy border border-ink/10 rounded-2xl flex items-center justify-center mb-6">
        ...
        {/* AI Companion panel */}
        {showChat && (
          <div className="flex flex-col bg-navy border-l border-ink/10 z-10 w-full md:w-[380px] lg:w-[420px] shrink-0">
            <div className="px-4 py-3 border-b border-ink/10 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="w-7 h-7 bg-gold/15 rounded-full flex items-center justify-center">
                ...
                <div>
                  <p className="text-sm font-medium text-ink">AI Companion</p>
                  <p className="text-[10px] text-muted-soft">Reading Ch. {chapter.number}</p>
                </div>
              </div>
              <button onClick={() => setShowChat(false)} className="text-muted-soft hover:text-ink transition-colors p-1 md:hidden">
            ...
            <div className="flex-1 overflow-y-auto reader-scroll px-4 py-4 space-y-4">
                ...
                <div key={i} className={`animate-message-in ${msg.role === 'user' ? 'flex justify-end' : ''}`}>
                  <div className={`rounded-2xl px-4 py-3 text-sm leading-relaxed max-w-[90%] ${
                    msg.role === 'user'
                      ...
                      : 'bg-navy-light text-ink/80 rounded-bl-md'
                  }`}>
              ...
              {isTyping && (
                <div className="animate-message-in">
                  <div className="bg-navy-light rounded-2xl rounded-bl-md px-4 py-3 inline-flex gap-1.5">
                    <div className="w-2 h-2 bg-muted-soft/30 rounded-full typing-dot" />
                    <div className="w-2 h-2 bg-muted-soft/30 rounded-full typing-dot" />
                    <div className="w-2 h-2 bg-muted-soft/30 rounded-full typing-dot" />
                  </div>
                </div>
              ...
              {showJourney && (
                <div className="animate-message-in">
                  <div className="bg-gold/10 border border-gold/25 rounded-2xl px-5 py-4">
                    <p className="text-[11px] font-semibold text-muted-soft uppercase tracking-[0.1em] mb-2">Your Journey</p>
                    {journeyLoading ? (
            ...
            <div className="p-3 border-t border-ink/10">
              <div className="flex items-center gap-2 bg-navy-light rounded-lg px-4 py-2.5 border border-ink/10 focus-within:border-gold transition-colors">
                <input
                  ref={chatInputRef}
                  type="text"
                  value={input}
                  ...
                  className="flex-1 bg-transparent text-sm text-ink placeholder:text-muted/40 outline-none disabled:opacity-40"
                />
                {!limitReached && (
                  <MicButton
                    currentText={input}
                ...
                <button
                  ...
                  className="text-gold hover:text-gold-light disabled:text-muted-soft disabled:cursor-not-allowed transition-colors p-1"
                >
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M2.01 21 23 12 2.01 3 2 10l15 2-15 2z"/></svg>
                </button>
              </div>
              <p className="text-[10px] text-muted-soft text-center mt-2">Personalized to your journey</p>
            </div>
          </div>
        )}
      </div>
