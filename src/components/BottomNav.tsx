...
const TABS = [
  {
    ...
    label: 'Library',
    icon: (active: boolean) => (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={active ? '#A86820' : '#3D5A2E'} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-cream border-t border-border safe-area-bottom">
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-xl transition-all min-w-[60px] ${
                isActive ? 'bg-gold/10' : 'hover:bg-ink/5'
              }`}
            >
              {tab.icon(isActive)}
              <span className={`text-[10px] font-medium ${isActive ? 'text-gold' : 'text-ink/40'}`}>
                {tab.label}
              </span>
            </Link>
          );
