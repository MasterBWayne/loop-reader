    ...
    <div className="min-h-screen bg-navy flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-md">
        ...
        <p className="text-sm text-ink/40 text-center mb-10">
          Choose a pace for <span className="text-ink/60">{bookTitle}</span>. You can change this later.
        </p>
        ...
        <div className="space-y-3">
          ...
          >
            <div className="flex items-start gap-4">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 mt-0.5 ${
                selected === 'guided' ? 'bg-gold/20' : 'bg-ink/5'
              }`}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={selected === 'guided' ? '#C97D2E' : '#6B5744'} strokeWidth="1.5">
                  <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
                </svg>
              </div>
              <div>
                <h3 className={`font-semibold text-sm mb-1 ${selected === 'guided' ? 'text-gold' : 'text-ink/80'}`}>
                  Guided pace
                </h3>
          ...
          >
            <div className="flex items-start gap-4">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 mt-0.5 ${
                selected === 'free' ? 'bg-gold/20' : 'bg-ink/5'
              }`}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={selected === 'free' ? '#C97D2E' : '#6B5744'} strokeWidth="1.5">
                  <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/>
                </svg>
              </div>
              <div>
                <h3 className={`font-semibold text-sm mb-1 ${selected === 'free' ? 'text-gold' : 'text-ink/80'}`}>
                  Read freely
                </h3>
                <p className="text-xs text-ink/40 leading-relaxed">
        ...
        <button
          ...
          disabled={!selected}
          className="w-full mt-8 bg-gold hover:bg-gold-light disabled:bg-ink/10 disabled:text-ink/30 text-ink font-semibold py-3 rounded-xl transition-all text-sm flex items-center justify-center gap-2"
        >
          Start reading
